...
module W_machine #(parameter WORDSIZE=1) (
      ...
      wire [WORDSIZE-1:0] partial1 = s1_Wtm2 + W_tm7;
      wire [WORDSIZE-1:0] partial2 = s0_Wtm15 + W_tm16;
      wire [WORDSIZE-1:0] Wt_next = pipe1 + pipe2;

      reg wt_valid;
      reg [WORDSIZE*16-1:0] W_stack_q;
      reg [WORDSIZE-1:0] pipe1, pipe2;
      ...
      always @(posedge clk) begin
            if (M_valid) begin
                ...
                wt_valid <= ~wt_valid;
                if (~wt_valid) begin
                    // Stage 1: latch partials when stack is stable
                    pipe1 <= partial1;
                    pipe2 <= partial2;
                end else begin
                    // Stage 2: shift stack with new Wt_next
                    W_stack_q <= W_stack_d;
                end
            end
        end
...
module sha256_block (
    ...
    always @(posedge clk)
    begin
        input_valid_delay <= input_valid;
    end
    
    sha256_K_machine sha256_K_machine (
        .clk(clk), .rst(input_valid_delay), .en(k_en), .K(Kj)
    );
